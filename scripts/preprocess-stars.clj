...
(defn -main []
  (println "Star Image Preprocessor")
  (println "=======================")

  (let [input-dir (File. "assets/stars")
        output-dir (File. "assets/stars-processed")]

    ;; Create output directory
    (when-not (.exists output-dir)
      (.mkdir output-dir)
      (println "Created output directory: assets/stars-processed"))

    ;; Get all PNG files
    (let [star-files (sort (.listFiles input-dir))
          png-files (filter #(.endsWith (.getName ^File %) ".png") star-files)]

      (println (format "\nProcessing %d star images..." (count png-files)))
      (println "Removing black backgrounds and saving to assets/stars-processed/")
      ...
      (println "Preprocessed images saved to: assets/stars-processed/"))))
