...
(defn plan-bar-chart
  ...
  (let [{:keys [x y width height]} (layout/bounds node)
        ...
        background-color (or (color/ensure background-color) (color/hsv 225 42.9 11.0 0.2))
        bar-color (or (color/ensure bar-color) (color/hsv 200.8 38.6 99.6))
        grid-color (or (color/ensure grid-color) (color/hsv 0 0 100 0.2))
        bars (vec values)
        count (count bars)
